...
//                }
//            }
            if (!ffp->real_record && (pkt->stream_index == is->video_stream)  && (pkt->flags & AV_PKT_FLAG_KEY)) { //遇到关键帧打开开关,quicktime比较严格
                MPTRACE("===== now III sample_rate =====\n");
                ffp->real_record = true;
                ffp_notify_msg2(ffp, FFP_MSG_REAL_RECORD, 0);
            }
...
void ffp_set_syncType_speed(FFPlayer *ffp, int type,float speed)
{
    if (!ffp)
        return;
    av_log(ffp, AV_LOG_INFO, "fix set speed: %f\n", speed);
    ffp->is->av_sync_type = type;
    set_clock_speed(&ffp->is->extclk,speed);
